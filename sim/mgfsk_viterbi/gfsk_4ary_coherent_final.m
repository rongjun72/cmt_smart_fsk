...
% ========================================================================
    function [s, sym_gray, bits, Ns] = generate_gfsk(sym_seq, Ns_total)
        ...
        bits(1:2:end) = bitget(sym_seq, 2);  % MSB
        bits(2:2:end) = bitget(sym_seq, 1);  % LSB
        Ns = Ns_in;
    end
...
if RUN_FLOOR_ANALYSIS
    ...
    for bt_idx = 1:length(BT_scan)
        ...
        r_ch_tmp = filter(ch_filter, s_tmp);
