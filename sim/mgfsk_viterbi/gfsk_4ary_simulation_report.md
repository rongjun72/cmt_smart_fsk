...
| 操作 | 原始循环 | 向量化 | 预期加速 |
|------|---------|--------|---------|
| 单次 t 的 branch 计算 | 16 次 `squeeze` + `norm` + 内积 | 1 次 `1×4 * 4×16` BLAS | **~12.8×**（实测） |
| 归一化 | 逐循环计算 | 预计算 `vecnorm` + 广播 | 零运行时开销 |
| survivor 选择 | 逐 state `if` 比较 | `max(..., [], 1)` | 向量化 SIMD |

> **实测加速**：$N_{sym} = 100000$ 时，原始三重循环 132.27 s → 向量化 10.36 s，**加速约 12.8×**。主要收益来自消除 MATLAB 的 `for` interpreter overhead 和 `squeeze`/`norm` 重复调用。
