...
for t in range(N):
    ...
    for ns in range(n_states):
        ...
        for c in candidates:
            ...
            report.append(f"      从状态 {c['from_state']} 输入 {'+1' if c['input'] == 0 else '-1':>3} | "
                         ...
                         f"分支度量 = |{log['received']:.3f} - {c['expected']:.3f}|² = {c['branch_metric']:.4f} | "
    ...
    report.append(f"\n    >>> 时刻 {t+1} 结束后的累计度量: " + 
                 ", ".join([f"PM[{s}]={pm_history[t+1, s]:.4f}" if pm_history[t+1, s] < INF else f"PM[{s}]=∞" for s in range(n_states)]))
