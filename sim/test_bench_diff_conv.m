...
q_span = 4;                  % span symbols of Guassian filter
fs_rx = sps*BW;              % RX sampling rate
...
tatart0 = tic;
%% --------------------- BER test bench initialize ----------------------%
% Convolutional code type selection: '(7,5)' | '(15,13)' | '(23,35)' | '(171,133)'
tb_conv =  [{'(7,5)'} {'(15,13)'} {'(23,35)'} {'(171,133)'}]; 
N_testbech = length(tb_conv);
BER_tb     = zeros(N_testbech,N_method,EbNo_len);
for idx_tb = 1:N_testbech
    conv_code_type = tb_conv{idx_tb};
    switch conv_code_type
        case '(7,5)'
            K_conv = 3; gen_poly = [7 5];     %% Constraint length Generator polynomial (octal)
        case '(15,13)'
            K_conv = 4; gen_poly = [15 13];
        case '(23,35)'
            K_conv = 5; gen_poly = [23 35];
        case '(171,133)'
            K_conv = 7; gen_poly = [171 133]; %% 64-stateOctal generator polynomial
        otherwise
    ...
    for idx_method = 3:(N_method-1)
        ...
        for idx_EbNo = 1:EbNo_len
            ...
            for idx_symb = 1:N_symb_loop
                ...
                drawnow;
            end % idx_symb = 0:N_symb_loop
        end % idx_EbNo = 1:EbNo_len
    end %idx_method = 3:(N_method-1)
    close(fd_proc);
    BER_tb(idx_tb,:,:) = BER_est;
    ttotal = toc(tatart0);
...
%% ---------------------------local functions-----------------------------%
function [fd_proc,ui_proc] = create_ber_table(EbNo_len,EbNo_dB,BER_est,Demod_method_list)
    fd_proc = figure;
    ui_proc = uitable(fd_proc,'Data',[zeros(EbNo_len,1) EbNo_dB' BER_est'],...
        'ColumnName',['idx_lp' 'EbNo' Demod_method_list],...
...
function [fd] = semilogy_curves(EbNo_dB,BER_est,BER_tb,idx_method,y_range,legend_str)
    ...
    semilogy(EbNo_dB,BER_est(2,:),'g--','LineWidth',2);          % BER_theory_coh
    for idx_tb = 1:N_testbech
        semilogy(EbNo_dB,squeeze(BER_tb(idx_tb,idx_method,:)),'o-','LineWidth',2);
    end
    grid on;
