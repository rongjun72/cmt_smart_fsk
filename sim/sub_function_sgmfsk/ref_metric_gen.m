function [ref_metric] = ref_metric_gen(Nsym_segment,sps,fs,fs_tx,fs_rx,sps_rx,F_dev,Flo,filt_dly)
    ...
    [~,rx_sig_rf,time_tx,~] = sgmfsk_modulator(Nsym_segment,'ref',sps,fs,fs_tx,F_dev,Flo,0);
    [rx_sig_lb,rx_tx_len]   = rx_ddc_mixer(rx_sig_rf,Flo,0,time_tx);
    [rx_sig_ds,rx_len]      = sgmfsk_decimation(rx_sig_lb,fs,fs_tx,0);
    [rx_cmix_out,time_rx]   = rx_cmix(rx_sig_ds,rx_len,0,fs_rx,0);
    ...
    lpf_abs(:,1) = abs(FILT.LPF_f1(mix_result(:,1)))*ratio_dev;%%/1.0489;%%1.1266; % f0
    lpf_abs(:,2) = abs(FILT.LPF_f2(mix_result(:,2))); % f1
    lpf_abs(:,3) = abs(FILT.LPF_f3(mix_result(:,3))); % f2
    lpf_abs(:,4) = abs(FILT.LPF_f4(mix_result(:,4)))*ratio_dev;%%/1.0489;%%1.1266; % f3
