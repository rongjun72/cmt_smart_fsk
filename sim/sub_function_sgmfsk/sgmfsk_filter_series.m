function delay_n = sgmfsk_filter_series(BW,fs,BR,fs_rx,g_TBW,g_span,g_sps,F_dev)
    ...
    switch BW
        case 1e3
            FLT.DEC_stage1 = CICDecimatorBypass('DecimationFactor',Dec1,'NumSections',5,'DifferentialDelay',1); fs_in1 = fs;
            FLT.DEC_stage2 = CICDecimatorBypass('DecimationFactor',Dec2,'NumSections',5,'DifferentialDelay',1); fs_in2 = fs/Dec1;
            FLT.DEC_stage3 = CICDecimatorBypass('DecimationFactor',Dec3,'NumSections',5,'DifferentialDelay',1); fs_in3 = fs/Dec1/Dec2;
            FLT.DEC_stage4 = CICDecimatorBypass('DecimationFactor',Dec4,'NumSections',5,'DifferentialDelay',1); fs_in4 = fs/Dec1/Dec2/Dec3;
            ...
            filt_dec_dly(1) = (Dec1~=1)*FLT.DEC_stage1.grpDelay()/fs_in1;
            filt_dec_dly(2) = (Dec2~=1)*FLT.DEC_stage2.grpDelay()/fs_in2;
            filt_dec_dly(3) = (Dec3~=1)*FLT.DEC_stage3.grpDelay()/fs_in3;
            filt_dec_dly(4) = (Dec4~=1)*FLT.DEC_stage4.grpDelay()/fs_in4;
    end
    
    %%% HBF filter
    switch BW
        case 1e3
            FLT.DEC_stage5 = FIRDecimatorBypass(Dec5,firhalfband(18,0.25));
            fs_hbFilt = fs/(Dec1*Dec2*Dec3*Dec4);
            filt_dec_dly(5) = (Dec5~=1)*FLT.DEC_stage5.grpDelay()/fs_hbFilt;
    ...
    fd1 = fvtool(FLT.chFilter,'Fs',fs_rx);  %%fvtool(FLT.chFilter,'Fs',fs_chFilt);
    fd1.Name = 'chFilt'; ax1 = fd1.CurrentAxes;
    title(ax1,sprintf('channel filter, [Wp,Ws]:[%.1f %.1f]',Wp*(fs_chFilt/2),Ws*(fs_chFilt/2)));
